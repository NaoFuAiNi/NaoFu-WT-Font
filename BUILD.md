...
1. 编译 C 主程序到 `debug\NaoFu WT Customize Font 2.10.exe`
...
```
debug\Launcher\bin\Debug\net8.0-windows\NaoFu WT Hub 2.10.exe
```
...
| 文件 | 如何生成 |
|------|----------|
| **NaoFu WT Customize Font 2.10.exe** | 在 `debug` 下执行 `scripts\build_c.bat`（推荐）或 `scripts\do_build.cmd`，也可在「x64 本机工具命令提示」里用 `scripts\build.bat` |
...
- **提示「未找到 C 程序所在目录」**  
  ...
  请先执行 `build_debug.ps1` 或至少执行一次 `scripts\build_c.bat` 生成 `NaoFu WT Customize Font 2.10.exe`，  
